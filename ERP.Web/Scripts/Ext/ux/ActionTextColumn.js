/**
 * @Description	文字+图标的guid单元格
 * @author		侯飞(544735725@qq.com)
 */
Ext.define('Ext.ux.Actiontextcolumn', {
            ...
            me.renderer = function(v, meta) {
                ...
                for (i = 0; i < l; i++) {
                    ...
                    v += '<a href="javascript:void(0);"' + 
                        ...
                        ' ' + (Ext.isFunction(item.getClass) ? item.getClass.apply(item.scope||me.scope||me, arguments) : (me.iconCls || '')) + '"' +
                        ((item.tooltip) ? ' data-qtip="' + item.tooltip + '"' : '') + '><img alt="' + (item.altText || me.altText) + '" src="' + (item.icon || Ext.BLANK_IMAGE_URL) +'" />' + (item.text || me.text) + '</a>';
                }
                return v;
            };
        }
